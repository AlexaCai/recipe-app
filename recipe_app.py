...
class Recipe(Base):
    ...
    # Method to print a well-formatted version of the recipes
    def __str__(self):
        output = (
            "-" * 25
            + "\n"
...
# Create the table of the model defined (class Recipe) above
...
def create_recipe():
    try:
        ...
        while (
            not name_line.strip()
            or not name_line.replace(" ", "").isalnum()
            or len(name_line) > 50
        ):
        ...
        # First check the number of ingredients to be entered (selected by users) is ok
        while (
            not number_of_ingredients.strip()
            or number_of_ingredients.isnumeric() != True
        ):
        ...
        for items in range(int(number_of_ingredients)):
            ...
            while (
                not ingredients_input.strip()
                or not ingredients_input.isalpha()
                or len(ingredients_input) > 255
                or ingredients_input in ingredients
            ):
            ...
            if len(ingredients_input) <= 255:
                if ingredients_input not in ingredients:
                    ...
                    print("Ingredient added to the recipe!")
        ...
        # Creating a new object from the Recipe model called recipe_entry using the details entered by users
        recipe_entry = Recipe(
            name=name,
...
def search_by_ingredients():
    try:
        ...
        print("-" * 25)
        print("Here are all available ingredients across all your recipes:")
        ...
        # to users. This is later used to ensure users select a number associated with an ingredient that \
        ...
        while (
            not number_picked.strip()
            or not number_picked.replace(" ", "").isnumeric()
            or any(int(num) == 0 for num in number_picked.split())
            or any(int(num) > max_index for num in number_picked.split())
        ):
            if not number_picked.strip():
                ...
                print("-" * 25)
                number_picked = input("Your number(s): ")
            elif any(int(num) == 0 for num in number_picked.split()) or any(
                int(num) > max_index for num in number_picked.split()):
                print("-" * 25)
        ...
        if not related_recipes:
            ...
            print("-" * 25)
            print("Here are all the recipe(s) that contain the ingredient(s) you've searched with. If you want more results, reducing the number of ingredients in your search might provide more recipes.")
            related_recipes = session.query(Recipe).filter(*conditions).all()
            print("-" * 25)
    ...
    try:
        ...
        if not recipes_list:
            ...
            for recipe in results:
                recipe_id = recipe[0]
                related_recipes = (
                    session.query(Recipe).filter(Recipe.id == recipe_id).all()
                )
                for recipe in related_recipes:
                    print(recipe)

            # Used to know which ids associated with recipes are in the list shown to users.
            # This is later used to ensure users select an id associated with a recipe that is within the \
            # the existing id options displayed on their UI (otherwise - error).
        ...
        while (
            not recipe_id_picked.strip()
            or recipe_id_picked.isnumeric() != True
            or int(recipe_id_picked) not in valid_ids
        ):
            if not recipe_id_picked.strip():
                ...
                print("-" * 25)
                recipe_id_picked = input("Your number(s): ")
            elif int(recipe_id_picked) not in valid_ids:
                print("-" * 25)
                print("The number you entered is not is associated with a recipe id number. You'll be brought back to the main menu. ")
                print("-" * 25)
                return
        ...
        while (
            not edit_number_chosen.strip()
            or edit_number_chosen.isnumeric() != True
            or not 0 < int(edit_number_chosen) <= 3
        ):
        ...
        if int(edit_number_chosen) == 1:
            ...
            while (
                not updated_name.strip()
                or not updated_name.replace(" ", "").isalnum()
                or len(updated_name) > 50
            ):
            ...
            # Ask users what they want to do next
            ...
            while (
                not updated_cooking_time.strip()
                or updated_cooking_time.isnumeric() != True
            ):
            ...
            # Note: 'return_ingredients_as_list()' in 'recipe_to_edit.return_ingredients_as_list()' below is \
            # a method defined in the Recipe class at the beginning of the file, which allow to return the \
            # list of ingredients as a list, and which can then be passeed as an argument in calculate_difficulty \
            # function. 
            # Update the difficulty level if necessary based on the cooking time change.
            ...
            while True:
                if int(edit_number_chosen) == 1:
                    if recipe_to_edit.ingredients == "":
                        ...
                        # is within the existing range of numbers displayed on their UI (otherwise - error)
                        ...
                        while (
                            not ingredient_number_to_remove.strip()
                            or ingredient_number_to_remove.isnumeric() != True
                            or any(int(num) == 0 for num in ingredient_number_to_remove.split())
                            or any(int(num) > max_index for num in ingredient_number_to_remove.split())
                        ):
                        ...
                        updated_difficulty = recipe_to_edit.calculate_difficulty(
                            recipe_to_edit.cooking_time,
                            recipe_to_edit.return_ingredients_as_list(),
                        )
                        recipe_to_edit.difficulty = updated_difficulty
                        print(recipe_to_edit)
                        ...
                        # Ask users what they want to do now
                ...
                if int(edit_number_chosen) == 2:
                    ...
                    for items in range(int(number_of_ingredients)):
                        ...
                        while (
                            ...
                            or not ingredients_input.isalpha()
                            or len(ingredients_input) > 255
                            or ingredients_input in ingredients_list
                            or ingredients_input in ingredients_added
                        ):
                            if not ingredients_input.strip():
                                print("-" * 25)
                                print("You must enter a value in this field (cannot stay empty). Please add the ingredient name.")
                        ...
                        # If the next ingredient users want to added is the same than one of the previous one \
                        # they've added, I'll throw an 'ingredient already exist' error message.
                        ...
                        # before users choose to add more in the recipe). Principle is the same here. 
                        # This code is used to ensure a same ingredient cannot be added twice.
                    ...
                    # Update the recipe's ingredients in a unique list
    ...
    try:
        ...
        if not recipes_list:
            ...
            while (
                not recipe_id_picked.strip()
                or recipe_id_picked.isnumeric() != True
                or int(recipe_id_picked) not in existing_recipe_ids
            ):
                if not recipe_id_picked.strip():
                    print("-" * 25)
